...
vec3 mat3_times_vec3(mat3 &mlhs, const vec3 &vrhs)
{

	float x = mlhs.m[0] * vrhs.v[0] + mlhs.m[3] * vrhs.v[1] + mlhs.m[6] * vrhs.v[2];
	float y = mlhs.m[1] * vrhs.v[0] + mlhs.m[4] * vrhs.v[1] + mlhs.m[7] * vrhs.v[2];
	float z = mlhs.m[2] * vrhs.v[0] + mlhs.m[5] * vrhs.v[1] + mlhs.m[8] * vrhs.v[2];
...
float length( const vec3 &v ) {
	return float(sqrt( v.v[0] * v.v[0] + v.v[1] * v.v[1] + v.v[2] * v.v[2] ));
}
...
vec3 heading_to_direction( float degrees ) {
	float rad = (float) (degrees * ONE_DEG_IN_RAD);
...
mat4 rotate_x_deg( const mat4 &m, float deg ) {
	// convert to radians
	float rad = float(deg * ONE_DEG_IN_RAD);
	mat4 m_r = identity_mat4();
	m_r.m[5] = float(cos( rad ));
	m_r.m[9] = float(-sin( rad ));
	m_r.m[6] = float(sin( rad ));
	m_r.m[10] = float(cos( rad ));
	return m_r * m;
}
...
mat4 rotate_y_deg( const mat4 &m, float deg ) {
	...
	float rad = float(deg * ONE_DEG_IN_RAD);
	mat4 m_r = identity_mat4();
	m_r.m[0] = (float)cos( rad );
	m_r.m[8] = (float)sin( rad );
	m_r.m[2] = (float)-sin( rad );
	m_r.m[10] = (float)cos( rad );
	return m_r * m;
}
...
mat4 rotate_z_deg( const mat4 &m, float deg ) {
	...
	mat4 m_r = identity_mat4();
	m_r.m[0] = (float)cos( rad );
	m_r.m[4] = (float)-sin( rad );
	m_r.m[1] = (float)sin( rad );
	m_r.m[5] = (float)cos( rad );
	return m_r * m;
}
...
mat4 perspective( float fovy, float aspect, float near, float far ) {
	...
	float range = float(tan( fov_rad / 2.0f ) * near);
	float sx = float(( 2.0f * near ) / ( range * aspect + range * aspect ));
	float sy = float(near / range);
	float sz = float(-( far + near ) / ( far - near ));
	float pz = float(-( 2.0f * far * near ) / ( far - near ));
...
versor slerp( versor &q, versor &r, float t ) {
	...
	float sin_half_theta = float(sqrt( 1.0f - cos_half_theta * cos_half_theta ));
	...
	float half_theta = (float)acos( cos_half_theta );
	float a = float(sin( ( 1.0f - t ) * half_theta ) / sin_half_theta);
	float b = float(sin( t * half_theta ) / sin_half_theta);
	for ( int i = 0; i < 4; i++ ) {
